...
do
	/usr/bin/sudo ssh root@192.168.1.1 "echo 'iptables -I FORWARD -s $2/24 -d $i -j ACCEPT' >> /etc/firewall/1d.fw" 2>&1
	/usr/bin/sudo ssh root@192.168.1.1 "iptables -I FORWARD -s $2/24 -d $i -j ACCEPT"
done
